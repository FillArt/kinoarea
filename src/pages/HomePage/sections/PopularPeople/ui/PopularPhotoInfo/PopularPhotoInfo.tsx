...
export const PopularPhotoInfo = ({data, place}: PopularPhotoInfoProps) => {
    ...
    const imgStyle = data?.profile_path
        ? {
            ...
            backgroundColor: 'rgba(0, 0, 0, 0.3)', // Это создает полупрозрачный фон для затемнения
            backgroundSize: "cover",
            backgroundPosition: "center",
            backgroundBlendMode: 'overlay', // Это позволяет смешать фоны
        }
        : {};

    return (
        <div style={imgStyle} className="h-full rounded-[10px] py-[12px] px-[16px] flex flex-col justify-between">
            <span className="text-[15px] text-decorTextColor">{place} место</span>
